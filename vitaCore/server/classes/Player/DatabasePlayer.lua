...
function DatabasePlayer:save()
    if getElementData(self, "isLoggedIn") == true then
        ...
        -- This is a vita3 leftover...
        local function calcWinrate(won, maps)
            local wonCount, mapsCount = self:getData(won), self:getData(maps)
            if wonCount > 0 and mapsCount > 0 then
                return math.round(wonCount/mapsCount*100,2)
            end
        end
